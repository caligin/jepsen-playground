Vagrant.configure("2") do |config|
  ...
  config.vm.provision "file", source: "deps/nomad", destination: "nomad"
  ...
  config.vm.provision "file", source: "nomad.hcl", destination: "nomad.hcl"
  config.vm.provision "file", source: "nomad.service", destination: "nomad.service"
  ...
  config.vm.provision "shell", inline: "cp nomad.hcl /etc/"
  config.vm.provision "shell", inline: "cp nomad.service /lib/systemd/system/"
  config.vm.provision "shell", inline: "ls /usr/bin/nomad || cp nomad /usr/bin/nomad"
  ...
  config.vm.provision "shell", inline: "systemctl enable nomad.service"
  config.vm.provision "shell", inline: "systemctl start nomad.service"
  ...
  config.vm.define "cluster01" do |cluster01|
    ...
    cluster01.vm.provision "shell", inline: "sed -i 's/address_here/172.28.128.11/g' /etc/nomad.hcl"
    cluster01.vm.provision "shell", inline: "systemctl restart nomad.service"
  end
  ...
  config.vm.define "cluster02" do |cluster02|
    ...
    cluster02.vm.provision "shell", inline: "sed -i 's/address_here/172.28.128.12/g' /etc/nomad.hcl"
    cluster02.vm.provision "shell", inline: "systemctl restart nomad.service"
  end
  ...
  config.vm.define "cluster03" do |cluster03|
    ...
    cluster03.vm.provision "shell", inline: "sed -i 's/address_here/172.28.128.13/g' /etc/nomad.hcl"
    cluster03.vm.provision "shell", inline: "systemctl restart nomad.service"
